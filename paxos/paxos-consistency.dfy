...
module Consistency {
    ...
    // lemma 4: if a leader proposes a value in the forced case, then that value must be from another leader with a smaller ballot
    ...
    // lemma 4: the nonforce case of lemma X: 
    ...
    // lemma 5: the nonforce case of lemma X && the leader c2 must have a ballot at least as large as c1
    ...
    // lemma X directly leads to the following Consistency lemma
    ...
    decreases s.leader_propose[c2]
    {
        if s.leader_ballot[c1] == s.leader_ballot[c2] { // trivial
            assert c1 == c2;
        } else { // bn1 < bn2, and since s.leader_propose[c2] > 0, we have 
            assert |s.promise_count[c2]| >= F + 1;
