...
module Invariants {
    ...
    requires type_ok(s)
    {
        ...
        && (forall a :: a in acceptors ==> (s.acceptor_state[a].value > 0 ==> s.acceptor_state[a].highest >=0 && s.acceptor_state[a].confirmed >= -1))
        && (forall a :: a in acceptors ==> (s.acceptor_state[a].value > 0 <==> s.acceptor_state[a].confirmed >= 0))
        && (forall a, bn, ballot, value :: a in acceptors && (PMsg(bn, ballot, value) in s.pmsgs[a]) ==> (ballot == -1 <==> value == 0))
        && (forall a :: a in acceptors && s.acceptor_state[a].value == 0 ==> s.cmsgs[a] == {})
        && (forall a :: a in acceptors ==> s.acceptor_state[a].highest >= s.acceptor_state[a].confirmed)
        && (forall a, bn, ballot, value  :: a in acceptors && (PMsg(bn, ballot, value) in s.pmsgs[a]) ==> bn <= s.acceptor_state[a].highest && ballot >= -1)
