 ...
 * There are a few variants of Paxos, such as Fast Paxos, Byzantine Paxos, and Raft, which are all different from the current model
 *
 ...
 * This is just a simple instance of the general Paxos protocol proposed by Leslie Lamport.
 * 
 * Simplification and abstractions made in the model:
 * -- We assume that an coordinator/leader picks a ballot when it becomes active (this event is atomic)
 * -- The ballot number to be selected is ever increasing, starting from 0; This implies every coordinator's ballot number is unique/distinct and the increment action is atomic
 * -- Each time a ballot is picked, such information is broadcast to all coordinators/leaders
 * -- A message is recorded in a global data structure, and may be delivered to each individual node by an individual "receive" (which is a read) action, for modelling asynchrony
 * -- We assume all messages are broadcast (since no party is malicious) though the communication is still asynchronous
 * -- No modelling of message loss (as it cannot be distinguished from being ignored by the receiver or the sender becomes faulty)
 * -- If a node becomes faulty/sleepy, it may wake up later and send other messages, but this is indistinguishable from delayed message sending; we assume no information is lost in this procedure
 ...
 *
 * We prove the safety condition (Consistency) in a separate module.
 */
...
module Paxos_protocol {
  ...
  // an internal state of an acceptor: the highest ballot promised so far, 
  // the accepted value with the (currently) confirmed ballot number
  datatype AState = AState(highest_ballot: int, confirmed_ballot: int, value: Proposal) // 
  // the message type from an acceptor in step 1b --- Promising
  datatype PMsg = PMsg(ballot: int, confirmed_ballot: int, value: Proposal)
  ...
  datatype TSState = TSState(
    ...
    leader_forced: map<Acceptor, Proposal>,   // recording the value in the force case in step 2a, initially 0
    leader_forced_ballot: map<Acceptor, int>, // recording the ballot value in the force case in step 2a, initially -1
    ...
    ballot: int,  // initially 0 and incremented each time when the current vallue is picked by a leader
    // states of acceptors
    ...
    // a collection of received PMsg promises for each leader. This is an auxiliary data structure.
    ...
    // an auxiliary data structure that records a leader that chooses a ballot
  ...
  requires type_ok(s)
  {
    ...
    && (forall c :: c in leaders ==> s.leader_forced_ballot[c] == -1)
    ...
    && (forall a :: a in acceptors ==> s.acceptor_state[a] == AState(-1, -1, 0))
  ...
  // which is modelled by (an acceptor) looking into the s.leader_ballot[c] component.
  predicate choose_ballot(s: TSState, s': TSState, c: Acceptor)
  requires type_ok(s) && type_ok(s') && c in leaders 
  {
    && s.leader_ballot[c] == -1 // leader c has not chosen any ballot before
    && s'.leader_ballot == s.leader_ballot[c:= s.ballot] // c is now associated with a ballot
    && s'.ballot == s.ballot + 1
  ...
  requires type_ok(s) && type_ok(s') && c in leaders && a in acceptors
  {
    ...
    && s.acceptor_state[a].highest_ballot < s.leader_ballot[c] // acceptor a has not yet promised a ballot greater than or equal to c 's ballot
    ...
    && (
      ...
      || (s.acceptor_state[a].value > 0 && (s'.pmsgs == s.pmsgs[a := s.pmsgs[a] + {PMsg(s.leader_ballot[c], s.acceptor_state[a].confirmed_ballot, s.acceptor_state[a].value)}]))
    )
    // acceptor a updates its highest ballot received so far --- 
    && s'.acceptor_state == s.acceptor_state[a := AState(s.leader_ballot[c], s.acceptor_state[a].confirmed_ballot, s.acceptor_state[a].value)]
    // all the other state components remain the same
  ...
  ghost predicate receive_response_1b(s: TSState, s': TSState, c: Acceptor, a: Acceptor, m: PMsg)
  requires type_ok(s) && type_ok(s') && c in leaders && a in acceptors //&& confirmed >= 0 && value > 0
  {
    //&& PMsg(s.leader_ballot[c], confirmed, value) in s.pmsgs[a]
    && m in s.pmsgs[a] && m.ballot == s.leader_ballot[c]
    ...
    && ( 
      ...
      || (m.confirmed_ballot > s.leader_forced_ballot[c] && (s'.leader_forced_ballot == s.leader_forced_ballot[c:= m.confirmed_ballot] && s'.leader_forced == s.leader_forced[c:= m.value]))
      // do not update s.leader_forced: accepter may have confirmed a lower ballot than leader_forced_ballot[c], or it has not confirmed yet 
      || (m.confirmed_ballot <= s.leader_forced_ballot[c] && (s'.leader_forced_ballot == s.leader_forced_ballot && s'.leader_forced == s.leader_forced)) 
    )
    && s'.promise_count == s.promise_count[c:= s.promise_count[c]+{a}] // adding acceptor a to leader c's promise_count
    && s'.received_promises == s.received_promises[c:= s.received_promises[c]+{m}]
    // all the other state components remain the same
    && s'.leader_ballot == s.leader_ballot
    && s'.leader_propose == s.leader_propose
    && s'.leader_decision == s.leader_decision
    ...
    // && confirmed <= s'.leader_forced_ballot[c] // this should be proved by a lemma
  }
  ...
  // scenario 2: if a leader with a ballot number bn has received ballots from F+1 acceptors and 
  // there is at least one message 1b that contains some previous confirmed value, i.e., s.leader_forced[c] > 0
  // the leader will propose the same value corresponding to the highest confirmed ballot (the force case). 
  ...
  {
    && s.leader_ballot[c] >= s.acceptor_state[a].highest_ballot // this '>' is possible as some previous (smaller ballot) propose messages may be lost
    && s.leader_ballot[c] >= 0
    ...
    && s'.acceptor_state == s.acceptor_state[a := AState(s.leader_ballot[c], s.leader_ballot[c], value)]
  ...
  ghost predicate receive_confirm_2b(s: TSState, s': TSState, c: Acceptor, a: Acceptor, m: CMsg)
  requires type_ok(s) && type_ok(s') && c in leaders && a in acceptors //&& value > 0
  {
    //&& CMsg(s.leader_ballot[c], value) in s.cmsgs[a] 
    && m in s.cmsgs[a]
    && m.ballot == s.leader_ballot[c]
    && m.value == s.leader_propose[c] > 0
  ...
  requires type_ok(s) && type_ok(s')
  {
    ...
    || (exists c, a, m :: c in leaders && a in acceptors && m in s.pmsgs[a] && receive_response_1b(s, s', c, a, m))
    || (exists c, value :: c in leaders && value > 0 && propose_value_2a(s, s', c, value))
    || (exists c, a, m :: c in leaders && a in acceptors && m in s.cmsgs[a] && receive_confirm_2b(s, s', c, a, m))
