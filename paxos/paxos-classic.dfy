...
module Paxos_protocol {
  ...
  datatype AState = AState(highest: int, value: Proposal, confirmed: int) // 
  ...
  requires type_ok(s) && type_ok(s') && c in leaders && a in acceptors
  {
    ...
    && (
      ...
      || (s.acceptor_state[a].value > 0 && (s'.pmsgs == s.pmsgs[a := s.pmsgs[a] + {PMsg(s.leader_ballot[c], s.acceptor_state[a].confirmed, s.acceptor_state[a].value)}]))
    )
    // acceptor a updates its highest ballot received so far --- 
    && s'.acceptor_state == s.acceptor_state[a := AState(s.leader_ballot[c], s.acceptor_state[a].value, s.acceptor_state[a].confirmed)]
    // all the other state components remain the same
  ...
  ghost predicate receive_response_1b(s: TSState, s': TSState, c: Acceptor, a: Acceptor, confirmed: int, value: Proposal)
  requires type_ok(s) && type_ok(s') && c in leaders && a in acceptors //&& confirmed >= 0 && value > 0
  {
    ...
    && ( 
      ...
      || (confirmed > s.leader_forced_ballot[c] && (s'.leader_forced_ballot == s.leader_forced_ballot[c:= confirmed] && s'.leader_forced == s.leader_forced[c:= value]))
  ...
  {
    && s.leader_ballot[c] >= s.acceptor_state[a].highest // this is possible as some previous (smaller balloted) messages may be lost
    && s.leader_ballot[c] >= 0
    ...
    && s'.acceptor_state == s.acceptor_state[a := AState(s.leader_ballot[c], value, s.leader_ballot[c])]
  ...
  requires type_ok(s) && type_ok(s')
  {
    ...
    || (exists c, a, confirmed, value :: c in leaders && a in acceptors && confirmed >=0 && value > 0 && receive_response_1b(s, s', c, a, confirmed, value))
