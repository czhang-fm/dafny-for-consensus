...
module AcceptorInvariants {
    ...
    {
        && (forall a, bn, v :: a in acceptors && CMsg(bn, v) in s.cmsgs[a] ==> s.acceptor_state[a].highest_ballot >= bn && s.acceptor_state[a].confirmed_ballot >= bn)
        ...
        && (forall a :: a in acceptors && s.acceptor_state[a].value > 0 ==> 
            (exists c :: c in leaders && s.leader_propose[c] == s.acceptor_state[a].value && s.leader_ballot[c] == s.acceptor_state[a].confirmed_ballot))
    ...
    ensures valid_acceptor(s')
    {
        ...
        assert forall m, c :: c in leaders && m in s.received_promises[c] && s.leader_ballot[c] >= 0 ==> (m.ballot == s.leader_ballot[c]);
        assert s.received_promises == s'.received_promises;
    }
    ...
    ensures valid_acceptor(s')
    {
        ...
        assert s.acceptor_state[a].value > 0 ==> PMsg(s.leader_ballot[c], s.acceptor_state[a].confirmed_ballot, s.acceptor_state[a].value) in s'.pmsgs[a];
        assert forall bn1, v1:: CMsg(bn1, v1) in s.cmsgs[a] && PMsg(s.leader_ballot[c], s.acceptor_state[a].confirmed_ballot, s.acceptor_state[a].value) in s'.pmsgs[a] ==> bn1 <= s.acceptor_state[a].confirmed_ballot;
    }
    ...
    requires receive_response_1b(s, s', c, a, m) && valid_acceptor(s)
    ensures valid_acceptor(s')
    {
    ...
    requires receive_confirm_2b(s, s', c, a, m) && valid_acceptor(s)
    ensures valid_acceptor(s')
    {}
