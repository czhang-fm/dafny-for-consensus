...
module Consistency {
    ...
    // lemma X helps to prove the Consistency lemma at the end
    ...
    ensures s.leader_propose[c1] == s.leader_propose[c2] // we need to prove the case for all c in leaders with their ballot in between of c1 and c2
    decreases s.leader_ballot[c2] - s.leader_ballot[c1]
    {
        if s.leader_ballot[c1] == s.leader_ballot[c2] { // trivial
            ...
            if s.leader_forced[c2] == 0 {
                ...
                var c3 := ForceCasePropose(s, c1, c2);
                assert s.leader_ballot[c1] <= s.leader_ballot[c3] < s.leader_ballot[c2];
                assert s.leader_propose[c3] == s.leader_propose[c2];
                X(s, c1, c3);
            }
        }
    }
    ...
    // ensures s.leader_decision[c1] == s.leader_decision[c2]
    {
        ...
        SubsetSize(s.decision_count[c1], set a | a in acceptors && CMsg(s.leader_ballot[c1], s.leader_propose[c1]) in s.cmsgs[a]);
        assert |set a | a in acceptors && CMsg(s.leader_ballot[c1], s.leader_propose[c1]) in s.cmsgs[a]| >= F + 1;
        SubsetSize(s.decision_count[c2], set a | a in acceptors && CMsg(s.leader_ballot[c2], s.leader_propose[c2]) in s.cmsgs[a]);
        assert |set a | a in acceptors && CMsg(s.leader_ballot[c2], s.leader_propose[c2]) in s.cmsgs[a]| >= F + 1;
        // var c :| c in leaders && |set a | a in acceptors && CMsg(s.leader_ballot[c], s.leader_propose[c]) in s.cmsgs[a]| >= F + 1;
        //&& forall c' :: c' in leaders && s.leader_ballot[c'] < s.leader_ballot[c] ==> (|set a | a in acceptors && CMsg(s.leader_ballot[c'], s.leader_propose[c']) in s.cmsgs[a]| <= F);
