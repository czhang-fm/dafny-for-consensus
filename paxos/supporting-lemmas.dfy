...
module SupportingLemmas {
    ...
    requires type_ok(s) && valid(s)
    requires c in leaders && a in acceptors && s.leader_forced[c] == 0 && a in s.promise_count[c]
    ensures exists m :: m in s.pmsgs[a] && m.confirmed_ballot == -1 && m.value == 0 && m.ballot == s.leader_ballot[c]
    // exists h, v :: PMsg(s.leader_ballot[c], h, v) in s.pmsgs[a] && h == -1 && v == 0 //PMsg(s.leader_ballot[c], -1, 0) in s.pmsgs[a]
    {} // this lemma should be equivalent to the next lemma. 
    // lemma 1.5
    ...
    {} // do we need to revise its presentation?
    ...
    ensures exists m :: m in s.received_promises[c2] && m.confirmed_ballot == s.leader_ballot[c3]
    //ensures exists a, v :: a in acceptors && PMsg(s.leader_ballot[c2], s.leader_ballot[c3], v) in s.pmsgs[a] && a in s.promise_count[c2]
    {
        ...
        var m :| m in s.received_promises[c2] && m in s.pmsgs[a];
        assert PMsg(s.leader_ballot[c2], m.confirmed_ballot, m.value) in s.pmsgs[a];  
        AcceptorConfirmPromise(s, a, c1, c2, m.confirmed_ballot, v1, m.value);
        var bn := m.confirmed_ballot;
        assert bn < s.ballot;
        c3 := s.ballot_mapping[bn];
    }

    // we prove the following auxiliary lemmas before the proof of lemma 8
    // lemma 7.5: a more general representation of lemma 8
    ...
    ensures forall m :: m in s.received_promises[c1] ==> m.confirmed_ballot <= s.leader_ballot[c2]
    ...
    ensures c2 in leaders && s.leader_forced[c1] == s.leader_propose[c2]
    {
        ...
        var m :| m in s.received_promises[c1] && m.confirmed_ballot == s.leader_forced_ballot[c1]; 
        c2 := s.ballot_mapping[m.confirmed_ballot];
    ...
    // lemma 8 (*): this proves the force case that given bn1 < bn2, v1 is confirmed by the system (by majority acceptors), if c2 proposes
    // then there exists c3 with bn1 <= bn3 and v2 == v3
    lemma ForceCasePropose(s: TSState, c1: Acceptor, c2: Acceptor) returns (c3: Acceptor)
    requires type_ok(s) && valid(s) && valid_acceptor(s) && valid_promise(s)
    requires c1 in leaders && s.leader_propose[c1] > 0
    requires c2 in leaders && s.leader_propose[c2] > 0 && s.leader_forced[c2] > 0 && s.leader_ballot[c1] < s.leader_ballot[c2]
    requires |set a | a in acceptors && CMsg(s.leader_ballot[c1], s.leader_propose[c1]) in s.cmsgs[a]| >= F + 1
    ...
    ensures c3 in leaders && s.leader_ballot[c1] <= s.leader_ballot[c3] < s.leader_ballot[c2]
    ensures c3 in leaders && s.leader_propose[c3] == s.leader_propose[c2]
    {
        var c4 := ForceCaseLarger(s, c1, c2);
        assert c4 in leaders && s.leader_ballot[c1] <= s.leader_ballot[c4];
        assert exists m :: m in s.received_promises[c2] && m.confirmed_ballot == s.leader_ballot[c4];
        c3 := ForceCaseLargestBallot(s, c2);
    }
