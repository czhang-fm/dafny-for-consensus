...
module Paxos_protocol {
  ...
  datatype TSState = TSState(
    ...
    // an auxiliary function mapping each acceptor to a ballot number which decides the acceptor's current confirmed value, initially -1
  ...
  requires type_ok(s) && type_ok(s') && c in leaders && a in acceptors
  {
    ...
    && (
      ...
      || (s.acceptor_state[a].value > 0 && (s'.pmsgs == s.pmsgs[a := s.pmsgs[a] + {PMsg(s.leader_ballot[c], s.acceptor_ballot[a], s.acceptor_state[a].value)}]))
    )
  ...
  requires type_ok(s) && type_ok(s') && c in leaders && a in acceptors && value > 0
  {
    ...
    && s'.acceptor_ballot == s.acceptor_ballot[a := s.leader_ballot[c]]
    // all the other state components remain the same
    && s'.leader_ballot == s.leader_ballot
    && s'.leader_forced == s.leader_forced
    && s'.leader_forced_ballot == s.leader_forced_ballot
  ...
  requires type_ok(s) && type_ok(s')
  {
    ...
    || (exists c, a, highest, value :: c in leaders && a in acceptors && receive_response_1b(s, s', c, a, highest, value))
